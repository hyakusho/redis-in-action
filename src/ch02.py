...
def clean_sessions(conn):
    while not QUIT:
        ...
        session_keys = []
        for token in tokens:
            session_keys.append('viewed:' + token.decode())
...
class TestCh02(unittest.TestCase):
    ...
    def test_login_cookies(self):
        ...
        LIMIT = 0
        t = threading.Thread(target=clean_sessions, args=(conn,), daemon=True)
        t.start()
        time.sleep(1)
        QUIT = True
        time.sleep(2)
    ...
    def test_cache_rows(self):
        ...
        t = threading.Thread(target=cache_rows, args=(conn,), daemon=True)
        t.start()
